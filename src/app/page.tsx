...
export default function HomePage() {
  ...
  const categoryImages: { [key: string]: string } = {
    ...
    audio:
      "https://shoopy.b-cdn.net/305289/SKU-1621_0-1730978221394.webp?format=webp",
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {getCategoryData?.categories?.map((item) => (
            <Link href={`/products?type=${item}`} key={item}>
              <Card
                key={"item"}
                className="group pt-0 cursor-pointer hover:shadow-lg transition-shadow"
              >
                <CardContent className="p-0">
      ...
      <p className="p-12 font-semibold">
        Hello, Just wanted to clarify a few things regarding the project setup.
        I was working with SSR (Server-Side Rendering), but since the code was
        running locally, I would have needed to either create custom API routes
        in Next.js or set up a proxy to handle the API calls properly. Given the
        time constraints, I prioritized implementing the core functionality and
        logic rather than focusing too much on the UI polish. My primary focus
        was to ensure the app is scalable, handles routing, pagination, and API
        integration effectively, and follows best practices. However, I'm
        confident that with a bit more time, the UI can be further enhanced
        without compromising performance or functionality. Looking forward to
        your feedback, and I’d be glad to walk you through the implementation
        choices I made. Thank you!
      </p>
    </div>
  );
}
